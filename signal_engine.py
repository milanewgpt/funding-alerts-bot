import logging
from dataclasses import dataclass
from config import FUNDING_THRESHOLD, PRICE_CHANGE_MIN, OI_CHANGE_MIN, SHORT_LIQ_MIN

log = logging.getLogger(__name__)


@dataclass
class Signal:
    symbol: str
    funding_prev: float
    funding_now: float
    price_prev: float
    price_now: float
    price_change_pct: float
    oi_prev: float
    oi_now: float
    oi_change_pct: float
    short_liq: float
    strong: bool  # True if short liquidation spike also present


def evaluate(current: list[dict], previous: list[dict]) -> list[Signal]:
    prev_map = {r["symbol"]: r for r in previous}
    signals = []

    for rec in current:
        symbol = rec["symbol"]
        prev = prev_map.get(symbol)
        if not prev:
            continue

        funding_now = rec.get("funding_rate")
        price_now = rec.get("price")
        oi_now = rec.get("oi")
        short_liq = rec.get("short_liq", 0) or 0

        funding_prev = prev.get("funding_rate")
        price_prev = prev.get("price")
        oi_prev = prev.get("oi")

        if any(v is None for v in [funding_now, price_now, oi_now, funding_prev, price_prev, oi_prev]):
            continue
        if price_prev == 0 or oi_prev == 0:
            continue

        # Core filter: funding must be negative enough
        if funding_now > FUNDING_THRESHOLD:
            continue

        price_change = (price_now - price_prev) / price_prev * 100
        if price_change < PRICE_CHANGE_MIN:
            continue

        oi_change = (oi_now - oi_prev) / oi_prev * 100
        if oi_change < OI_CHANGE_MIN:
            continue

        strong = short_liq >= SHORT_LIQ_MIN

        signals.append(Signal(
            symbol=symbol,
            funding_prev=funding_prev,
            funding_now=funding_now,
            price_prev=price_prev,
            price_now=price_now,
            price_change_pct=price_change,
            oi_prev=oi_prev,
            oi_now=oi_now,
            oi_change_pct=oi_change,
            short_liq=short_liq,
            strong=strong,
        ))
        log.info(f"Signal: {symbol} funding={funding_now:.4f}% price={price_change:+.2f}% OI={oi_change:+.2f}%")

    return signals
